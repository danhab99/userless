...
export const Thread: ThreadRelationResolvers = {
  files: (_obj, { root }) => {
    return db.thread.findUnique({ where: { id: root?.id } }).files()
  },
  parent: (_obj, { root }) => {
    return db.thread.findUnique({ where: { id: root?.id } }).parent()
  },
  replies: (_obj, { root }) => {
    return db.thread.findUnique({ where: { id: root?.id } }).replies()
  },
}
