...
export interface Fetcher {
  url: string;
  fetch: (path: string, args?: Record<string, string>) => Promise<string>;
  fetchContent: (path: string, args?: Record<string, string>) => Promise<Content>;
}

export interface BaseFetcher extends Fetcher {
  base: string;
  fetchFrom: (path: string, args?: Record<string, string>) => Promise<string>;
}

export function createFetcher(url: string): Fetcher {
  const fetcher = {
    url,
    async fetch(path: string, args?: Record<string, string>): Promise<string> {
    ...
    async fetchContent(path: string, args?: Record<string, string>): Promise<Content> {
...
export function createBaseFetcher(url: string, base: string): BaseFetcher {
  ...
  return {
    ...fetcher,
    base,
    async fetchFrom(path: string, args?: Record<string, string>) {
